...
sbt "runMain minimalcpu.MyTopLevelVerilog"

// To generate the VHDL from the example
sbt "runMain minimalcpu.MyTopLevelVhdl"

// To run the testbench
sbt "runMain minimalcpu.MyTopLevelSim"
```

* The example hardware description is into `hw/spinal/minimalcpu/MyTopLevel.scala`
* The testbench is into `hw/spinal/minimalcpu/MyTopLevelSim.scala`
...
You might want to change the project name, which is currently `minimalcpu`. To do so (let's say your actual project name is `myproject`; it must be all lowercase with no separators):

* Update `build.sbt` and/or `build.sc` by replacing `minimalcpu` by the name of your project `myproject` (1 occurrence in each file). The better is to replace in both (it will always work), but in some contexts you can keep only one of these two files:
...
* Put all your scala files into `hw/spinal/myproject/` (remove the unused `hw/spinal/minimalcpu/` folder)
...
```sh
// To generate the Verilog from the example
mill minimalcpu.runMain minimalcpu.MyTopLevelVerilog

// To generate the VHDL from the example
mill minimalcpu.runMain minimalcpu.MyTopLevelVhdl

// To run the testbench
mill minimalcpu.runMain minimalcpu.MyTopLevelSim
```
