...
import minimalcpu.plugin.Plugin
  ...
  def build(): Unit = {
    ...
    for (stageIndex <- 0 until stages.length; stage = stages(stageIndex)) {
      stage.inputs.keysIterator.foreach(key =>
        if (!insertedStageable.contains(key)) {
          SpinalWarning(s"Missing input insert for '${key.getName()}' in stage '${stage.getName()}'")
          missingInserts += key
        }
      )
      stage.outputs.keysIterator.foreach(key =>
        if (!insertedStageable.contains(key)) {
          SpinalWarning(s"Missing output insert for '${key.getName()}' in stage '${stage.getName()}'")
          missingInserts += key
        }
      )
    }
    ...
    for (stageIndex <- 0 until stages.length; stage = stages(stageIndex)) {
      stage.inputs.keysIterator.foreach(key =>
        inputOutputKeys.getOrElseUpdate(key, new KeyInfo).addInputStageIndex(stageIndex)
      )
      stage.outputs.keysIterator.foreach(key =>
